...
trait ThreadedConsumer extends Closeable with LazyLogging {
  ...
  def startConsumers(): Unit = {
    ...
    var i = 0
    consumers.foreach { c =>
      executor.execute(new ConsumerRunnable(c, Duration.ofMillis(frequency), String.format(format, Int.box(i))))
      i += 1
    }
  ...
  class ConsumerRunnable(consumer: Consumer[Array[Byte], Array[Byte]], frequency: Duration, id: String) extends Runnable {
