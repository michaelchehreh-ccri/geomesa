...
import org.locationtech.geomesa.kafka22.KafkaDataStore
...
class KafkaGetTypeNamesCommandExecutor(override val params: KafkaGetTypeNamesParams) extends Runnable with KafkaDataStoreCommand {
  ...
  override def execute(): Unit = {
    withDataStore { ds =>
      ds.getTypeNames.foreach(Command.output.info)
    }
    if (params.zkPath == null) {
      Command.user.info(s"Running List Features without zkPath...")

      KafkaDataStore.withZk(params.zookeepers) { zk =>
        zk.getAllTopics.filter(_.contains('-')).foreach(printZkPathAndTopicString(zk.zkClient, _))
      }
    } else {
